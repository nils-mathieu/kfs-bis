...
pub unsafe fn outb(port: u16, value: u8) {
    asm!("out {:x}, {}", in(reg) port, in(reg_byte) value, options(nomem, nostack, preserves_flags));
}
...
pub unsafe fn inb(port: u16) -> u8 {
    let value: u8;
    asm!("in {:x}, {}", in(reg) port, out(reg_byte) value, options(nomem, nostack, preserves_flags));
    value
}
